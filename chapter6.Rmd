...
As seen in tables, structure of dataframes are quite different, although they include the same information. A new variable "week" has been added to long form dataframe, RATSL. 
...
Here a random variable intercept (1 | ID) has been added and linear mixed model analysis (lmer function) has been executed.
...
Here is another model with random intercept and random slope model (Time | ID).
...
#### ANOVA of "Random intercept and random slope model" and "Random intercept model"

Here we can see results of ANOVA for two aforementioned models.
...
#### Random intercept and random slope model with the interaction
...
#### ANOVA of "Random intercept and random slope model with interaction" and "Random intercept and random slope model"

Here again ANOVA for two latest models.

Chi squared is 4.3719 with 3 df's and p = 0.22. That is, differences between groups are not statistically significant. 
