...
- Better students consume less alcohol
- Good family relations decrease drinking 
- Drinking may cause absences

According to hyphotheses, the selected explanatory variables are sex, final grade, family relations and absences. According to first hyphothesis, all analyses have been divided to two groups, females and males.
...
table(alc$sex, alc$high_use) %>% addmargins() %>% prop.table()

```

##### Relationship between alcohol consumption and final grades

In males high alcohol consumption influences more to grades than in females. In females the difference between groups is insignificant. 

```{r echo=FALSE, message=FALSE, warning=FALSE}
# initialize a plot of high_use and grades
g1 <- ggplot(alc, aes(x = high_use, y = G3, col = sex))
g1 + geom_boxplot() + xlab("Alcohol consumption low / high") + ylab("Grades")
```
...
All chosen variables are statistically significant. Especially sex is most significant of all and its odd ratio was the highest (2.84). This means that propability for high alcohol consumption in males is almost three times higher than in females. None of the included variables include 1 in confidental interval, which means they all have meaningful influence to the model. 

```{r echo=FALSE, message=FALSE, warning=FALSE, comment=''}
# Logistic regression model with chosen variables
m <- glm(high_use ~ famrel + absences + sex + G3, data = alc, family = "binomial")
...
With chosen variables, predictive power of model (penalty) is ~ 0.25 (TRUE-FALSE + FALSE-TRUE pairs summed). This means that about 3/4 of predictions are correct (TRUE-TRUE + FALSE-FALSE pairs summed).
...
This model have approximately the same prediction error using 10-fold cross-validation compared to the model introduced in DataCamp (which had about 0.26 error). 
...
The highest amount of predictors is in my original model introduced earlier. Other models are reduced combinations (N=17) of original model. In 10-fold cross validation, the smallest penalty loss was in model with following predictors: Sex, Family relations and Absences. 

```{r echo=FALSE, message=FALSE, comment=''}

# find the model with glm()
m1 <- glm(high_use ~ famrel + absences + sex + G3, data = alc, family = "binomial")
...
m5 <- glm(high_use ~ absences + sex + G3, data = alc, family = "binomial")
m6 <- glm(high_use ~ famrel + absences+ G3, data = alc, family = "binomial")
m7 <- glm(high_use ~ famrel + absences + sex, data = alc, family = "binomial")
m8 <- glm(high_use ~ absences, data = alc, family = "binomial")
m9 <- glm(high_use ~ absences + sex + G3, data = alc, family = "binomial")
m10 <- glm(high_use ~ famrel + sex + G3, data = alc, family = "binomial")
...
m15 <- glm(high_use ~ sex + G3, data = alc, family = "binomial")
m16 <- glm(high_use ~ famrel + G3, data = alc, family = "binomial")
m17 <- glm(high_use ~ absences + G3, data = alc, family = "binomial")
m18 <- glm(high_use ~ G3, data = alc, family = "binomial")


model_vector = c("m1", "m2", "m3", "m4", "m5", "m6", "m7", "m8", "m9", "m10", "m11", "m12", "m13", "m14", "m15", "m16", "m17", "m18")
